...
$selectType = "SELECT applying_for,count(applying_for) applied FROM `vb_users` where applying_for in ('merit','scholarship') and approval_status !='Duplicate'  group by applying_for";
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $results.='<tr>
            <td><a href="/admin/users.php?type='.$row["applying_for"].'">' . ucwords($row["applying_for"]) . '</a></td>
            <td>' . $row["applied"] . '</td>           
        </tr> ';
    }
    $results.='<tr>
